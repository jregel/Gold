...
#  combine the model data csvs into one dataframe
os.chdir(os.path.join(os.path.dirname(os.path.realpath('__file__')),'../data'))
print(os.getcwd())

dfs = [read_csv(f, index_col=[0], parse_dates=[0])
        for f in os.listdir(os.getcwd()) if f.endswith('csv')]

final_df = concat(dfs, axis=1, join='inner').sort_index()
...
# load dataset
dataset = read_csv('out.csv', header=0, index_col=0)
